...
BaseFacebook.prototype.restserver = function restserver(params, callback) {
  ...
  var host = this.getApiHost(method);

  delete params['method'];
  
  this.oauthRequest(host, '/restserver.php', params, method, cb.returnToCallback(callback, false, function(response) {
    try {
...
BaseFacebook.prototype.graph = function graph(/* path, method, params, callback */) {
  ...
  var self = this;
  this.oauthRequest(domain, path, params, method, cb.returnToCallback(callback, false, function(response) {
    try {
...
BaseFacebook.prototype.getAccessTokenFromCode = function getAccessTokenFromCode(code, redirectUri, callback) {
  if (!code) {
    ...
    this.oauthRequest('graph.facebook.com', '/oauth/access_token', 'GET' , {
      client_id: this.getAppId(),
 ...
 */
BaseFacebook.prototype.oauthRequest = function oauthRequest(host, path, params, method, callback) {
  var self = this;
  ...
  function next() {
    ...
    self.makeRequest(host, path, params, method, callback);
  }
};
 ...
 * @param callback
 */
BaseFacebook.prototype.makeRequest = function makeRequest(host, path, params, method, callback) {
  requestUtil.requestFacebookApi( host, 443, path, params, method, this.fileUploadSupport, callback);
};
